...
The code was developed and tested on Windows with MATLAB. The scripts are expected to be platform independent on systems where MATLAB and the required toolboxes are available.
...
No non-standard hardware is required to run the simulations.

Typical installation time is less than 30 minutes on a standard desktop computer, excluding MATLAB installation.
...
### Fig.2--Fig.4 parameter analysis
...
### Weak-nonlinearity compensation
...
## Expected outputs and run time

Each script generates figures and, where applicable, source-data files in the corresponding `outputs` folder.

Expected outputs include:

```text
PNG or FIG figures
MAT source-data files
CSV summary files when generated by the script
```

Typical run time ranges from several minutes to tens of minutes depending on the script and computer configuration. Scripts involving k-Wave simulations or PAWS genetic-algorithm optimization may take longer.
...
The code was prepared for manuscript review and focuses on figure-generation and mechanism-validation workflows rather than packaged software deployment.

---

## License

This code is released under the MIT License, an OSI-approved open-source license.

There are no additional restrictions on code use.
